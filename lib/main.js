                    ...
                    db.query(sql, (err, rows) => {
                        inquirer.prompt([
                            ...
                            .then(({ firstName, lastName, roleTitle }) => {
                                getRoleIDNoManager(firstName, lastName, roleTitle, 'NULL')

                            })
                    })
                } else if (managerConfirm === 'Yes') {
                    let sql = `SELECT CONCAT(employees.first_name, ' ', employees.last_name) AS name, employees.first_name FROM employees`;
                    db.query(sql, (err, rows) => {
